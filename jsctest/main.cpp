...
//  jsctest
//
//  Created by Wang Shuwei on 2019/7/3.
...
int main(int argc, const char* argv[]) {
    ...
    ctx.clear_exception();
    const auto result3 = ctx.eval_script("x => x**2").to_object().call(10).to_number();
    if (ctx.ok()) {
        std::cout << result3 << std::endl;
    }
    
    ctx.clear_exception();
    ctx.root()["run"] = ctx.callable([](auto ctx, auto this_object, auto args, auto exception) {
        if (args.size() == 0) {
            *exception = ctx.val("To few arguments!");
        } else if (args.size() == 1) {
            return ctx.val(args[0].to_number() + 10);
        } else {
            *exception = ctx.val("To many arguments!");
        }
    ...
    const auto result5 = ctx.get_exception().to_string();
    std::cout << result5 << std::endl;
    
    ctx.clear_exception();
    std::ifstream input{"~/tmp/jsctest/test/sudoku_v1.js"};
    std::stringstream buffer;
    buffer << input.rdbuf();
    std::cout << buffer.str().length() << std::endl;
    auto start = std::chrono::high_resolution_clock::now();
    ctx.eval_script(buffer.str());
    if (!ctx.ok()) {
